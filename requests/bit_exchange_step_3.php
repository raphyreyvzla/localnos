...
if(!isValidEmail($bit_u_field_1)) {
...
} elseif($receive == "Bank Transfer VES" && empty($bit_u_field_2)) {
	$data['status'] = 'error';
	$data['msg'] = error("$lang[error_39]");
} elseif($receive == "Bank Transfer VES" && empty($bit_u_field_3)) {
	$data['status'] = 'error';
	$data['msg'] = error("$lang[error_40]");
} elseif($receive == "Bank Transfer VES" && empty($bit_u_field_4)) {
	$data['status'] = 'error';
	$data['msg'] = error("$lang[error_41]");
}  elseif($receive == "Bank Transfer VES" && empty($bit_u_field_5)) {
	$data['status'] = 'error';
	$data['msg'] = error("$lang[error_42]");
}  elseif($receive == "Bank Transfer VES" && empty($bit_u_field_6)) {
	$data['status'] = 'error';
	$data['msg'] = error("$lang[error_43]");
	...
	if($receive == "Bank Transfer") {
		$account_data = '<tr>
               <td><span class="pull-left">'.$lang[id].'</span></td>
  ...
  elseif($receive == "Bank Transfer VES") {
		$account_data = '<tr>
							...
							<td><span class="pull-left">'.$lang[your_bank_name].'</span></td>
							<td><span class="pull-right">'.$row[u_field_8].'</span></td>
					</tr><tr>
							<td><span class="pull-left">'.$lang[your_bank_account].'</span></td>
							<td><span class="pull-right">'.$row[u_field_5].'</span></td>
					</tr><tr>
